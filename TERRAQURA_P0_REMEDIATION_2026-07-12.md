...
| Item | Owner | Status |
|---|---|---|
| P0.1 signed **Aethelred Ecosystem Release Manifest** (chain IDs, token symbol, precompile address/ABI, endpoints, compatible release tags) | Aethelred protocol repo | **Open** — TerraQura already consumes a single manifest package and pins 7331/7332/AETHEL/0x0900; conflicting values in Cruzible/ZeroID/NoblePay/public materials must be reconciled at the protocol repo. |
| Digital Seal AIP finalization (0x0900 vs draft 0x0500, ABI) | Aethelred protocol repo | **Open** — TerraQura's `ISeal` is vendored from the aethelred repo; the public AIP must be updated to match the implemented precompile. |
