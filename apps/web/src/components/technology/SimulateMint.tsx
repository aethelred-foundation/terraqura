...
import { cryptoRandomHex } from "@/lib/random";
        ...
        <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white/60 text-sm font-data">
              PROOF-OF-PHYSICS SIMULATOR
            </span>
          </div>
          {state !== "idle" && (
            <button
              onClick={handleReset}
              className="text-white/60 hover:text-white/60 text-xs font-data transition-colors"
            >
              RESET
            </button>
          )}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label
                ...
                className="block text-white/55 text-xs font-data uppercase tracking-wider mb-2"
              >
                Energy Consumed (kWh)
              </label>
              <input
          ...
          {state === "idle" && (
            <button
              ...
              className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-white/5 disabled:text-white/55 text-white font-semibold rounded-lg transition-all duration-200 disabled:cursor-not-allowed font-data text-sm"
            >
              RUN VERIFICATION
            </button>
          )}
          ...
          <AnimatePresence mode="wait">
            {state === "validating" && (
              ...
              >
                <TerminalLine
                  text="Initializing Proof-of-Physics Engine..."
                  delay={0}
                />
                <TerminalLine
                  text={`Energy input: ${energyInput} kWh`}
                  delay={0.3}
                />
                <TerminalLine
                  text={`CO2 captured: ${co2Input} tonnes`}
                  delay={0.5}
                />
                <TerminalLine
                  text={`Computing efficiency ratio: ${efficiency?.toFixed(1)} kWh/tonne`}
                  delay={0.8}
                />
                <TerminalLine
                  text="Validating against physical constraints [200-600 kWh/t]..."
                  delay={1.1}
                />
                <div className="flex items-center gap-2 mt-4">
                  <div className="w-4 h-4 border-2 border-yellow-500 border-t-transparent rounded-full animate-spin" />
                  <span className="text-yellow-400/60 text-xs font-data">
                    VERIFYING...
                  </span>
                </div>
              </motion.div>
              ...
              >
                <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-emerald-400 font-bold text-sm font-data">
                      VERIFICATION PASSED
                    </span>
                  </div>
                  <div className="space-y-2 text-xs font-data">
                    <div className="flex justify-between">
                      ...
                      <span className="text-emerald-400">
                        {efficiency?.toFixed(1)} kWh/tonne
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="p-4 rounded-lg bg-white/[0.03] border border-white/[0.08]">
                  <p className="text-white/60 text-xs font-data mb-2 uppercase tracking-wider">
                ...
                <div className="text-center">
                  <p className="text-white/55 text-xs font-body italic">
              ...
              >
                <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-red-400 font-bold text-sm font-data">
                      VERIFICATION FAILED
                    </span>
                  </div>
                ...
                <div className="text-center">
                  <p className="text-white/60 text-xs font-body">
                    This is why TerraQura credits can never be forged. The
                    physics must add up.
                  </p>
                </div>
              </motion.div>
...
function TerminalLine({ text, delay }: { text: string; delay: number }) {
  return (
    ...
    >
      <span
        className="text-emerald-500/50 font-data text-xs"
        aria-hidden="true"
      >
        &gt;
      </span>
