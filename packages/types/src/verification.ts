...
export function calculateEfficiencyFactor(
  ...
  if (purityPercentage < MIN_PURITY_PERCENTAGE) {
    return {
      factor: 0,
      isValid: false,
      reason: "Purity below minimum threshold",
    };
  }
  ...
  if (kwhPerTonne < MIN_KWH_PER_TONNE) {
    return {
      factor: 0,
      isValid: false,
      reason: "Suspiciously efficient - potential fraud",
    };
  }
  ...
  if (kwhPerTonne <= OPTIMAL_KWH_PER_TONNE) {
    ...
    const bonus =
      ((OPTIMAL_KWH_PER_TONNE - kwhPerTonne) * SCALE) /
      (OPTIMAL_KWH_PER_TONNE - MIN_KWH_PER_TONNE);
    factor = SCALE + bonus / 20; // Max 5% bonus
  } else {
    // Worse than optimal - apply penalty
    const penalty =
      ((kwhPerTonne - OPTIMAL_KWH_PER_TONNE) * SCALE) /
      (MAX_KWH_PER_TONNE - OPTIMAL_KWH_PER_TONNE);
